...
class AlternatingLeastSquares(MatrixFactorizationBase):
    ...
    def __init__(
        self,
        item_features: np.ndarray=None, # Array of size i * f for item_features
        regularization=0.01,
        alpha=1.0,
        dtype=np.float32,
        use_native=True,
        ...
        if item_features is None:
            raise ValueError("item_factors cannot be None")
        # parameters on how to factorize
        self.item_factors = item_features
        self.factors = item_features.shape[1]
    ...
    def fit(self, user_items, show_progress=True):
        ...
        with tqdm(total=self.iterations, disable=not show_progress) as progress:
            ...
            for iteration in range(self.iterations):
                ...
                # solver( # Should not need this as this calls for the least_squares to update Ciu?
                #     Ciu,
                #     self.item_factors,
                #     self.user_factors,
                #     self.regularization,
                ...
                # )
                progress.update(1)
                # print(self.calculate_training_loss)
